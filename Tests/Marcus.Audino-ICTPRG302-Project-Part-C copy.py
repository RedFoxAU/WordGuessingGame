...
def history_fails():
    ...
    return history_count, history_wins, fails, avg_score
...
def game_report(history_words, history_wins, avg_score, attempts, user_name):
    ...
    print(f"Percentage of Games Won: {percent_game_won}% | Average number of guesses to Win:", round(avg_score))
  #  save_history(f"{attempts}-{user_name.upper()}")
...
#     percent_game_won = round(history_wins / history_word_count * 100, 2)  # Calculate percentage of games won
#     print(f"Total Games Played: {history_word_count}\nTotal Games Won: {history_wins}\nTotal Games Lost: {history_losses}")
#     print(f"Percentage of Games Won: {percent_game_won}% | Average number of guesses to Win:", round(avg_score * 100, 2))
#   #  save_history(f"{attempts}-{user_name.upper()}")
...
def play_game(dev_debug, user_name, history_wins, avg_score, historic_attempts, history_count):  # DEBUG mode - remove for production
       ...
       # avg_score = avg_score # Calculate average attempts on winning games
        ...
        if guess == target_word: # Check if the guess is correct in one guess
            if attempts == 1:
                ...
                # print(f"Percentage of Games Won: {percent_game_won}% | Average number of guesses to Win:", round(avg_score * 100, 2))
                # save_history(f"{attempts}-{user_name.upper()}")
                game_report(history_words, history_wins, avg_score, attempts, user_name)  # Save scores and user name in score sheet for future ranking feature
            else:
    ...
    game_report(history_words, history_wins, avg_score, attempts, user_name)
"""
This is the main function of the game.
"""
def main():
    fails, history_wins, history_count, avg_score = history_fails()
    history_words = load_history(HISTORY_FILE)
    attempts = 0
    # avg_score =
    ...
    while True:
     ...
     #   avg_score = average_attempts(history_words)
        ...
        play_game(dev_debug, user_name, history_wins, avg_score, fails, history_count) # DEBUG mode - remove for production
